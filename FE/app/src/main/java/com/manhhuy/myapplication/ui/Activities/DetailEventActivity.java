...
public class DetailEventActivity extends AppCompatActivity {
        ...
        if (ApiConfig.isOrganizer()) {
            ...
            binding.btnShare.setOnClickListener(v -> {
                if (eventData != null && "ACTIVE".equals(eventData.getStatus())) {
                    ...
                    Toast.makeText(this, "Chỉ có thể gửi thông báo cho sự kiện đang diễn ra", Toast.LENGTH_SHORT)
                            .show();
                }
            });
        } else {
    ...
    private void checkUserRoleAndShowRegisterButton() {
        // Check if event is expired first
        boolean isEventExpired = Boolean.TRUE.equals(eventData.getIsExpired());

        if (isEventExpired) {
            // Event expired - show disabled button
        ...
        if (isRegistered && registrationId != null) {
            ...
            binding.btnRegisterEvent.setVisibility(View.VISIBLE);
            binding.btnRegisterEvent.setEnabled(true);
            ...
            binding.btnRegisterEvent.setBackgroundTintList(getResources().getColorStateList(R.color.button_blue));
        } else {
    ...
    private void cancelRegistration() {
        if (isRegistering || registrationId == null)
            return;
    ...
    private void uploadFileImmediately(Uri fileUri) {
        ...
        try {
            ...
            call.enqueue(new Callback<RestResponse<Map<String, String>>>() {
                @Override
                public void onResponse(Call<RestResponse<Map<String, String>>> call,
                        Response<RestResponse<Map<String, String>>> response) {
                    dismissProgressDialog();
                    if (response.isSuccessful() && response.body() != null && response.body().getData() != null) {
                        ...
                        try {
                            String errorBody = response.errorBody() != null ? response.errorBody().string()
                                    : "Unknown error";
                            Toast.makeText(DetailEventActivity.this, "Tải lên thất bại: " + errorBody,
                                    Toast.LENGTH_LONG).show();
                        } catch (Exception e) {
                ...
                public void onFailure(Call<RestResponse<Map<String, String>>> call, Throwable t) {
                    ...
                    selectedFileUri = null;
                    tvSelectedFileName.setText("Chưa chọn tệp");
                    Toast.makeText(DetailEventActivity.this, "Lỗi kết nối: " + t.getMessage(), Toast.LENGTH_SHORT)
                            .show();
                }
            });
        ...
        call.enqueue(new Callback<RestResponse<Map<String, Object>>>() {
            ...
            public void onResponse(Call<RestResponse<Map<String, Object>>> call,
                ...
                if (response.isSuccessful()) {
                    ...
                    Toast.makeText(DetailEventActivity.this, "Gửi thất bại: " + response.code(), Toast.LENGTH_SHORT)
                            .show();
                }
            }
